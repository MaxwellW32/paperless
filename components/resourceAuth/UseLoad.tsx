...
import { clientCallEnsureCanAccessResource } from '@/serverFunctions/handleAuth'
import { expectedResourceType, resourceAuthResponseType, resourceAuthType } from '@/types'
...
export default function useResourceAuth(resource: expectedResourceType | undefined) {
    ...
    useEffect(() => {
        const search = async () => {
            try {
                //run when the auth is ready - and when a value for resource is actually passed
                if (resourceAuth === undefined || resource === undefined) return

                const newResourceAuthObj = await clientCallEnsureCanAccessResource(resource, resourceAuth)
                resourceAuthResponseSet(newResourceAuthObj)
