...
export async function ensureCanEditDepartment({ departmentIdBeingAccessed, allowElevatedAccess = false }: departmentAuthType): Promise<authAccessLevelResponseType> {
...
export async function ensureCanEditCompany({ companyIdBeingAccessed }: companyAuthType): Promise<authAccessLevelResponseType> {
    //who can edit
    //company users - company admins can edit company
    //admin - all edit
    ...
    //user is from Egov making a change
    if (session.user.fromDepartment) throw new Error("department user can't edit company")
    ...
    if (seenUserToCompany.companyAccessLevel !== "admin") throw new Error("only company admins can make changes")

    //auth role validation
    return { session, accessLevel: seenUserToCompany.companyAccessLevel }
}
export async function canUserEditCompany(companyAuth: companyAuthType): Promise<boolean> {
    try {
        await ensureCanEditCompany(companyAuth)
        return true

    } catch (error) {
        console.log(`$canUserAccesCompany error`, error);
        return false
    }
}


export async function ensureCanEditClientRequest(clientReqAuth: clientRequestAuthType): Promise<authAccessLevelResponseType> {
    //who can edit
    //company users - can edit checklist if from company - specific forms - manual if waiting on client/email out
    //department users - can edit checklist if department can manage - and only if manual 
    //admin - all edit
    //check checklist at run time to determine access / display different fields of the checklist depending on role
    ...
    if (session.user.fromDepartment) {
        ...
        //check if user really in department they sent
        ...
        //ensure from same company of request
...
export async function canUserEditClientRequest(clientReqAuth: clientRequestAuthType): Promise<boolean> {
    try {
        await ensureCanEditClientRequest(clientReqAuth)
        return true

    } catch (error) {
        console.log(`$canUserAccesClientRequest error`, error);
        return false
    }
}
